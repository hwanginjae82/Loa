...
const normalizeCatalog = (catalog) => {
  ...
  const knownIds = new Set(initialCatalog.map((raid) => raid.id));
  const merged = [
    ...initialCatalog.map((defaultRaid) => incoming.find((raid) => raid.id === defaultRaid.id) ?? defaultRaid),
    ...incoming.filter((raid) => !knownIds.has(raid.id)),
  ];
  return sortRaidCatalog(merged.map((raid) => ({ ...raid, hexColor: getRaidColor(raid) })));
};
...
function RaidCard({ instance, catalog, roster, day, weekDays, conflictMap, isDragging, canMoveUp, canMoveDown, onSlotClick, onTimeChange, onMoveDay, onDragStart, onDragEnd, onDrop, onMoveUp, onMoveDown }) {
...
function ScheduleView({ members, catalog, schedule, setSchedule, weekDays, weekStart, canGoPrevious, canGoNext, onPreviousWeek, onCurrentWeek, onNextWeek, onCopyNextWeek, onToggleUnavailable }) {
  ...
  const displayDayRaids = useMemo(() => [...dayRaids].sort((left, right) => (left.order ?? Number.MAX_SAFE_INTEGER) - (right.order ?? Number.MAX_SAFE_INTEGER)), [dayRaids]);
  ...
  const sortDayByTime = () => setSchedule((current) => ({ ...current, [selectedDay]: withOrder([...(current[selectedDay] ?? [])].sort((left, right) => raidTimeSortValue(left.startTime) - raidTimeSortValue(right.startTime))) }));
...
function RosterSyncModal({ member, onClose, onSave }) {
  ...
  const saveSelected = () => {
    ...
    onSave(characters.filter((character) => selectedIds.includes(character.id)).map((character, index) => ({
      ...character,
...
function MemberRosterCard({ member, schedule, catalog, weekDays, onRename, onColorChange, onToggleActive, onSync, onToggleRole, onToggleGold }) {
  return <article className={`member-roster-card ${member.active === false ? "inactive" : ""}`}>
    ...
    <div className="character-grid">{member.characters.map((character) => {
      const assignments = getCharacterAssignments(schedule, catalog, character.id, null, weekDays);
      return <section key={character.id} className={character.earnsGold === false ? "no-gold" : ""}><div><strong>{character.name}</strong><span>{character.className}</span></div><b>Lv. {character.itemLevel}</b><button className={character.role === "서폿" ? "support" : "dealer"} disabled={character.className !== "발키리"} title={character.className === "발키리" ? "딜러/서폿 전환" : "직업 고정 역할"} onClick={() => onToggleRole(character.id)}>{character.role}</button><button className={`gold-toggle ${character.earnsGold === false ? "off" : "on"}`} title="이 캐릭터의 레이드 골드 획득 여부" onClick={() => onToggleGold(character.id)}>{character.earnsGold === false ? "비골드" : "골드"}</button><CharacterAssignmentSummary assignments={assignments} weekDays={weekDays} /></section>;
    })}</div>
  </article>;
}

function MembersView({ members, setMembers, schedule, catalog, weekDays }) {
  ...
  const saveRoster = (characters) => { setMembers((current) => current.map((member) => member.id === syncMemberId ? { ...member, characters } : member)); setSyncMemberId(null); };
...
function PersonalScheduleView({ members, schedule, catalog, weekDays, weekStart }) {
  ...
  const characterSummaries = selectedCharacters.map((character) => {
    ...
    const recommendationCandidates = [...bestEligibleByName.values()]
      ...
      .slice(0, 2);
    return { character, assignments, uniqueAssignments, selectedPersonalRaids, recommendationCandidates, count: uniqueAssignments.length + selectedPersonalRaids.length, guildCount: uniqueAssignments.length, duplicateCount: assignments.length - uniqueAssignments.length };
  });
  ...
  return <section className="personal-page">
    ...
    <div className="personal-summary">
      ...
      <div className="personal-raid-progress">{characterSummaries.map(({ character, count, guildCount, duplicateCount, selectedPersonalRaids, recommendationCandidates }) => <div key={character.id} className={duplicateCount ? "over" : count > 3 ? "extra" : count === 3 ? "complete" : "incomplete"}><span>{character.name}{character.earnsGold === false && <small>비골드</small>}</span><strong>{count}/3</strong><b>{duplicateCount ? `동일 레이드 ${duplicateCount}개 중복${count < 3 ? ` · ${3 - count}회 남음` : ""}` : count > 3 ? `골드 완료 · ${count - 3}회 추가 참여(골드 없음)` : count === 3 ? "완료" : `${3 - count}회 남음`}</b><small className="raid-count-source">길드 {guildCount} · 개인 {selectedPersonalRaids.length}</small>{selectedPersonalRaids.length > 0 && <div className="selected-personal-raids">{selectedPersonalRaids.map((raid) => <button key={raid.id} onClick={() => togglePersonalRaid(character.id, raid.id)} style={{ borderColor: getRaidColor(raid) }} title="개인 선택 취소"><span>{raid.name} {raid.difficulty}</span><b>{formatNumber(raid.gold)} G</b><i>×</i></button>)}</div>}{count < 3 && <div className="personal-raid-recommendations"><em>따로 갈 레이드 선택</em>{recommendationCandidates.map((raid) => <button key={raid.id} onClick={() => togglePersonalRaid(character.id, raid.id)} style={{ borderLeftColor: getRaidColor(raid) }}><span>{raid.name} {raid.difficulty}</span><small>Lv. {raid.minLevel} · {formatNumber(raid.gold)} G</small></button>)}</div>}</div>)}</div>
    </div>
    <div className="personal-week">{weekDays.map((day) => {
      ...
      return <article key={day.key} className={`${day.key === "sun" ? "sunday" : ""} ${day.key === "mobile" ? "mobile-call" : ""}`}><header><div><strong>{day.key === "mobile" ? "모바출" : `${day.label}요일`}</strong><span>{day.date}</span></div>{myMember?.unavailable.includes(day.key) && <b>불참 날짜</b>}</header><div>{entries.length ? entries.map((entry) => <section key={`${entry.character.id}-${entry.instanceId}-${entry.slotIndex}`} style={{ borderLeftColor: entry.raidColor }}><strong>{entry.character.name}</strong><span>{entry.raidName} {entry.difficulty}</span><small>{entry.startTime || "시간 미정"} · {entry.character.role} · Lv. {entry.character.itemLevel}</small><em>{entry.character.earnsGold === false ? "비골드" : `${formatNumber(entry.gold)} 골드`}</em></section>) : <p>일정 없음</p>}</div></article>;
    })}</div>
  </section>;
}
...
export function App() {
  ...
  useEffect(() => { localStorage.setItem("loa-raid-schedule-v1", JSON.stringify(scheduleData)); }, [scheduleData]);
  useEffect(() => { localStorage.setItem("loa-active-week-v1", activeWeekStart); }, [activeWeekStart]);
  const saveCloudField = (field, value) => {
    if (!supabase || !cloudReadyRef.current) return undefined;
    const fingerprint = JSON.stringify(value);
    if (fingerprint === lastSyncedRef.current[field]) return undefined;
    setCloudStatus("saving");
    const timer = setTimeout(async () => {
      const { error } = await supabase.from("raid_board_state").update({ [field]: value, updated_at: new Date().toISOString() }).eq("id", "guild-main");
      if (error) {
        setCloudStatus("error");
        setCloudMessage(error.message);
        return;
      }
      lastSyncedRef.current[field] = fingerprint;
      setCloudStatus("connected");
      setCloudMessage("");
    }, 500);
    return () => clearTimeout(timer);
  };
  useEffect(() => saveCloudField("members", members), [members]);
  useEffect(() => saveCloudField("catalog", catalog), [catalog]);
  useEffect(() => saveCloudField("schedule", scheduleData), [scheduleData]);
  ...
  const setCurrentSchedule = (updater) => setScheduleData((current) => {
  ...
  const toggleUnavailable = (memberId, dayKey) => setScheduleData((current) => {
    const existingWeek = current.weeks[activeWeekStart] ?? { raids: emptyRaidDays(), unavailableByMember: {} };
  ...
  const copyToNextWeek = () => {
    ...
    const hasTargetSchedule = target && scheduleDayKeys.some((key) => target.raids[key]?.length);
    if (hasTargetSchedule && !window.confirm("다음 주에 이미 일정이 있습니다. 현재 주 일정으로 덮어쓸까요?")) return;
    setScheduleData((current) => {
      const source = current.weeks[activeWeekStart] ?? { raids: emptyRaidDays() };
      const copiedRaids = JSON.parse(JSON.stringify(source.raids));
