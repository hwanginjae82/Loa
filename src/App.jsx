  ...
  return <section className="personal-page">
    ...
    <div className="personal-summary">
      ...
      <div className="personal-raid-progress">{characterSummaries.map(({ character, count, guildCount, duplicateCount, selectedPersonalRaids, plannedRaids, recommendationCandidates }) => <div key={character.id} className={duplicateCount ? "over" : count > 3 ? "extra" : count === 3 ? "complete" : "incomplete"}>
        <span>{character.name}{character.earnsGold === false && <small>비골드</small>}</span><strong>{count}/3</strong>
    ...
    <div className="personal-week">{weekDays.map((day) => {
      ...
      return <article key={day.key} className={`${day.key === "sun" ? "sunday" : ""} ${day.key === "mobile" ? "mobile-call" : ""}`}><header><div><strong>{day.key === "mobile" ? "모바출" : `${day.label}요일`}</strong><span>{day.date}</span></div>{myMember?.unavailable.includes(day.key) && <b>불참 날짜</b>}</header><div>{entries.length ? entries.map((entry) => <section key={`${entry.character.id}-${entry.instanceId}-${entry.slotIndex}`} className={isRaidCompleted(entry.character.id, entry.raidName) ? "checked" : ""} style={{ borderLeftColor: entry.raidColor }}><strong>{entry.character.name}</strong><span>{entry.raidName} {entry.difficulty}</span><div className="personal-raid-meta"><small>{entry.startTime || "시간 미정"}</small><em>{entry.character.earnsGold === false ? "비골드" : `${formatNumber(entry.gold)} 골드`}</em></div><input className="personal-complete-check" type="checkbox" checked={isRaidCompleted(entry.character.id, entry.raidName)} aria-label={`${entry.character.name} ${entry.raidName} ${entry.difficulty} 완료`} onChange={() => toggleRaidCompleted(entry.character.id, entry.raidName)} /></section>) : <p>일정 없음</p>}</div></article>;
    })}</div>
  </section>;
}
