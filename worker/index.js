...
async function lostArkProfile(request, env) {
  const characterName = new URL(request.url).searchParams.get("characterName")?.trim();
  if (!characterName) return json({ message: "캐릭터명을 입력해주세요." }, 400);
  if (!env.LOSTARK_API_JWT) return json({ message: "로스트아크 API 키가 설정되지 않았습니다." }, 503);
  const authorization = env.LOSTARK_API_JWT.toLowerCase().startsWith("bearer ") ? env.LOSTARK_API_JWT : `bearer ${env.LOSTARK_API_JWT}`;
  const response = await fetch(`https://developer-lostark.game.onstove.com/armories/characters/${encodeURIComponent(characterName)}/profiles`, { headers: { accept: "application/json", authorization } });
  if (!response.ok) return json({ message: `캐릭터 정보 조회 실패 (${response.status})` }, response.status);
  const profile = await response.json();
  return json({ name: profile.CharacterName ?? characterName, itemLevel: parseItemLevel(profile.ItemAvgLevel), combatPower: parseItemLevel(profile.CombatPower) });
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/lostark/profile") return await lostArkProfile(request, env);
